...
(or another format if using B<--datatype>) you know from the PRINT and GPRINT
commands.
...
[B<--datatype> B<numeric>|B<timestamp>|B<duration>]

Set how data should be interpreted.
...
Data should be interpreted as unix timestamps (number of seconds
since 1970-01-01 00:00:00 UTC). The y-axis and print/gprint values are then
formatted using strftime (with default format '%Y-%m-%d %H:%M:%S'). See
also B<--units-lengths>.

=item B<duration>: Data should be interpreted as duration (in milliseconds)

Formatting of y-axis and print/gprint values is done in a printf-like way:
