...
Binary prefix y-axis scaling. Implies --units=iec
...
With this option set to si, y-axis values on logarithmic graphs will be scaled to
...
With this option set to iec, y-axis values on all graphs will be scaled to
the appropriate units (K, M, etc.) instead of using SI or exponential notation.
